...
- 代码体积控制（同 HelloAGENTS 编码原则）：文件/类预警 300 行、超 400 行强制评估（单一内聚职责可记录理由保留，多职责才必须拆），函数预警 40 行、强制拆分 60 行。例外：生成代码、大型测试夹具、迁移脚本、协议常量表、数据类（Entity/DO/DTO/VO）、ORM 映射与转换器
