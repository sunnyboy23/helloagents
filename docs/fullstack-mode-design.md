...
全栈模式（`~fullstack`）的目标是：在保持 HelloAGENTS 原有工作流一致性的前提下，提供面向多项目协同的统一编排能力。
...
- 功能：配置解析、依赖分析、调度、状态反馈、文档同步
- 正确性：拓扑层级正确、触发逻辑正确、状态统计准确
