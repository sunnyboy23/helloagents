...
- 代码体积控制：
  ...
  - 函数/方法强制拆分阈值（超过后必须在完成功能后按职责拆分）：函数/方法 60 行
  - 文件/类强制评估阈值（超过 400 行必须评估并给出结论）：单一内聚职责可记录理由后保留，承载多职责才必须按职责拆分
  - 例外类型：生成代码、大型测试夹具、迁移脚本、协议常量表、数据类（Entity/DO/DTO/VO 等字段聚合体）、ORM 映射与转换器
