...
2. 审查维度: 安全（OWASP Top 10、注入、硬编码密钥）、质量（可读性、重复、错误处理）、性能（复杂度、资源占用）、代码体积控制（文件/类超300行须评估拆分、超400行须强制拆分；函数超40行须评估拆分、超60行须强制拆分；例外: 生成代码、大型测试夹具、迁移脚本、协议常量表）、视觉/UX（含 UI 代码时，按技术栈适配: 样式可维护性、组件一致性、可访问性、响应式/多尺寸适配）
