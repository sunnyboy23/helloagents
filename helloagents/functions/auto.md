...
当收到 ~auto [需求描述]:
  ...
  核心维度未全部充分 → 追问回合（按 {EVAL_MODE}，含最后一轮追问确认合一）:
    输出追问信息 [→ G4 确认信息格式]
...
当用户选择执行方式:
  ...
  委托执行:
    ...
    按 R2 标准流程进入阶段链（见下方"阶段路由"）

  交互式执行:
    WORKFLOW_MODE = INTERACTIVE
    按 R2 标准流程进入阶段链，关键决策点 ⛔ END_TURN（方案选择、失败处理）
...
```yaml
R2 标准流程阶段链:
  1. 按 G7 按需读取表"R2 进入方案设计（入口）"加载 → 方案设计（含上下文收集，按 TASK_COMPLEXITY 决定是否多方案对比）
  2. 按 G7 按需读取表"R2 进入开发实施（入口）"加载 → 开发实施
  3. 验收: 按 G8 流程级验收规则执行
...
| 级别难以判定 | 默认使用 R2 标准流程（~auto 强制 R2） |
